%%%%%%%%%%%%数据处理   求每一帧与平均的min,四分位,max为特征（5*6=30维）
Double_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\Double100K_200K\';
Single_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\Singles200K\';
 %%%%%%   Double_featire
...
for iis=3:length(subfolders1)  
      ...
      for i=1:m
            Double_feature_i(i,1)=abs(Double_data(i,1)-Double_data_ave(1));
          Double_feature_i(i,2)=abs(Double_data(i,2)-Double_data_ave(2));
          Double_feature_i(i,3)=abs(Double_data(i,10)-Double_data_ave(10));
         Double_feature_i(i,4)=abs(Double_data(i,11)-Double_data_ave(11));
         Double_feature_i(i,5)=abs(Double_data(i,26)-Double_data_ave(26));
         Double_feature_i(i,6)=abs(Double_data(i,27)-Double_data_ave(27));
      end
     Double_feature_i=sort(Double_feature_i);
      Double_feature_wei(1,1:6)=Double_feature_i(1,:);%%%%%%  min
      index_left=floor(m/2);%%%%%%%   floor:向下取整
      index_right=ceil(m/2);%%%%%%%   floor:向上取整
      Double_feature_wei(1,7:12)=(Double_feature_i(floor(index_left/2),:)+Double_feature_i(ceil(index_left/2),:))/2;%%%%%%  上四分位
      Double_feature_wei(1,13:18)=(Double_feature_i(index_left,:)+Double_feature_i(index_right,:))/2;%%%%%%中位数
      Double_feature_wei(1,19:24)=(Double_feature_i(index_right+floor((m-index_right+1)/2)-1,:)+Double_feature_i(index_right+ceil((m-index_right+1)/2)-1,:))/2;%%%%%%  下四分位
      Double_feature_wei(1,25:30)=Double_feature_i(m,:);%%%%%%  max
     mm=mm+1;
     Double_feature(mm,:)=Double_feature_wei;
end   
 %%%%%%  Single_featire
...
for iis=3:length(subfolders2)  
      ...
      for i=1:m
         Single_feature_i(i,1)=abs(Single_data(i,1)-Single_data_ave(1));
         Single_feature_i(i,2)=abs(Single_data(i,2)-Single_data_ave(2));
         Single_feature_i(i,3)=abs(Single_data(i,10)-Single_data_ave(10));
         Single_feature_i(i,4)=abs(Single_data(i,11)-Single_data_ave(11));
         Single_feature_i(i,5)=abs(Single_data(i,26)-Single_data_ave(26));
         Single_feature_i(i,6)=abs(Single_data(i,27)-Single_data_ave(27));
      end
      Single_feature_i=sort(Single_feature_i);
      Single_feature_wei(1,1:6)=Single_feature_i(1,:);%%%%%%  min
      index_left=floor(m/2);%%%%%%%   floor:向下取整
      index_right=ceil(m/2);%%%%%%%   floor:向上取整
      Single_feature_wei(1,7:12)=(Single_feature_i(floor(index_left/2),:)+Single_feature_i(ceil(index_left/2),:))/2;%%%%%%  上四分位
      Single_feature_wei(1,13:18)=(Single_feature_i(index_left,:)+Single_feature_i(index_right,:))/2;%%%%%%中位数
      Single_feature_wei(1,19:24)=(Single_feature_i(index_right+floor((m-index_right+1)/2)-1,:)+Single_feature_i(index_right+ceil((m-index_right+1)/2)-1,:))/2;%%%%%%  下四分位
      Single_feature_wei(1,25:30)=Single_feature_i(m,:);%%%%%%  max
     mm=mm+1;
     Single_feature(mm,:)=Single_feature_wei;
end   
