%%%%%%%%%%%%数据处理   求每一帧与平均差的四分位为特征（4*6=24维）从小到大排序
...
for iis=3:length(subfolders1)  
      ...
      wei=4;
      index=floor(m/(wei+1));%%%%%%%四分位，分五份，取4个数   floor:向下取整
      nn=1;
      for w=1:wei         
        Double_feature_wei(1,nn:nn+5)=Double_feature_i(index*w,:);%%%%%%%%%%   wei*6维
        nn=nn+6;
      end
     mm=mm+1;
...
for iis=3:length(subfolders2)  
      ...
      nn=1;
      for w=1:wei         
        Single_feature_wei(1,nn:nn+5)=Single_feature_i(index*w,:);%%%%%%%%%%   wei*6维
        nn=nn+6;
      end
     mm=mm+1;
