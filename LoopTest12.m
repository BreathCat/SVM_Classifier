%%%%%%%%%%%%数据处理   求求5种I帧PU类型中随机四帧与平均的差为特征（4*6=24维）
...
for iis=3:length(subfolders1)  
      ...
      for i=1:m
         Double_feature_i(i,:)=abs(Double_data(i,:)-Double_data_ave);
      end
      %%%%%%randperm(m, n)，函数作用：从1~m中随机产生n个不重复的数。 
      randArray=randperm(25,4);
      randArray_ALL(iis-2,:)=randArray;
         Double_feature_rand(1,1:5)=Double_feature_i(randArray(1),:);
         Double_feature_rand(1,6:10)=Double_feature_i(randArray(2),:);
         Double_feature_rand(1,11:15)=Double_feature_i(randArray(3),:);
         Double_feature_rand(1,16:20)=Double_feature_i(randArray(4),:);
     mm=mm+1;
...
for iis=3:length(subfolders2)  
      ...
      for i=1:m
         Single_feature_i(i,:)=abs(Single_data(i,:)-Single_data_ave);
      end
         Single_feature_rand(1,1:5)=Single_feature_i( randArray_ALL(iis-2,1),:);
         Single_feature_rand(1,6:10)=Single_feature_i(randArray_ALL(iis-2,2),:);
         Single_feature_rand(1,11:15)=Single_feature_i(randArray_ALL(iis-2,3),:);
         Single_feature_rand(1,16:20)=Single_feature_i(randArray_ALL(iis-2,4),:);
     mm=mm+1;
