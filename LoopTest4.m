%%%%%%%%%%%%数据处理   求随机四帧与平均的差为特征（4*6=24维）
Double_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\intra_pre_modes\Double100K_200K\';
Single_folder= 'E:\梁晓云\video forensics\35intra_pred_modes\intra_pre_modes\Singles200K\';
...
for iis=3:length(subfolders1)  
      ...
      %%%%%%randperm(m, n)，函数作用：从1~m中随机产生n个不重复的数。 
      randArray=randperm(25,4);
      randArray_ALL(iis-2,:)=randArray;
         Double_feature_rand(1,1:6)=Double_feature_i(randArray(1),:);
         Double_feature_rand(1,7:12)=Double_feature_i(randArray(2),:);
         Double_feature_rand(1,13:18)=Double_feature_i(randArray(3),:);
         Double_feature_rand(1,19:24)=Double_feature_i(randArray(4),:);
     mm=mm+1;
     Double_feature(mm,:)=Double_feature_rand;
end   
 %%%%%%  Single_featire
...
for iis=3:length(subfolders2)  
      ...
      for i=1:m
         ...
         Single_feature_rand(1,1:6)=Single_feature_i(randArray_ALL(iis-2,1),:);
         Single_feature_rand(1,7:12)=Single_feature_i(randArray_ALL(iis-2,1),:);
         Single_feature_rand(1,13:18)=Single_feature_i(randArray_ALL(iis-2,1),:);
         Single_feature_rand(1,19:24)=Single_feature_i(randArray_ALL(iis-2,1),:);
     mm=mm+1;
     Single_feature(mm,:)=Single_feature_rand;
end   
