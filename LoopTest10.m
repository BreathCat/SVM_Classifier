%%%%%%%%%%%%数据处理   求PU类型的min,四分位,max为特征（5*6=30维）
Double_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\I_PU\Double100K_200K\';
Single_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\I_PU\Singles200K\';
 %%%%%%   Double_featire
...
for iis=3:length(subfolders1)  
      ...
      Double_feature_i=sort(Double_data);
      Double_feature_wei(1,1:5)=Double_feature_i(1,:);%%%%%%  min
      index_left=floor(m/2);%%%%%%%   floor:向下取整
      index_right=ceil(m/2);%%%%%%%   floor:向上取整
      Double_feature_wei(1,6:10)=(Double_feature_i(floor(index_left/2),:)+Double_feature_i(ceil(index_left/2),:))/2;%%%%%%  上四分位
      Double_feature_wei(1,11:15)=(Double_feature_i(index_left,:)+Double_feature_i(index_right,:))/2;%%%%%%中位数
      Double_feature_wei(1,16:20)=(Double_feature_i(index_right+floor((m-index_right+1)/2)-1,:)+Double_feature_i(index_right+ceil((m-index_right+1)/2)-1,:))/2;%%%%%%  下四分位
      Double_feature_wei(1,21:25)=Double_feature_i(m,:);%%%%%%  max
     mm=mm+1;
...
for iis=3:length(subfolders2)  
      ...
      Single_feature_i=sort(Single_data);
      Single_feature_wei(1,1:5)=Single_feature_i(1,:);%%%%%%  min
      index_left=floor(m/2);%%%%%%%   floor:向下取整
      index_right=ceil(m/2);%%%%%%%   floor:向上取整
      Single_feature_wei(1,6:10)=(Single_feature_i(floor(index_left/2),:)+Single_feature_i(ceil(index_left/2),:))/2;%%%%%%  上四分位
      Single_feature_wei(1,11:15)=(Single_feature_i(index_left,:)+Single_feature_i(index_right,:))/2;%%%%%%中位数
      Single_feature_wei(1,16:20)=(Single_feature_i(index_right+floor((m-index_right+1)/2)-1,:)+Single_feature_i(index_right+ceil((m-index_right+1)/2)-1,:))/2;%%%%%%  下四分位
      Single_feature_wei(1,21:25)=Single_feature_i(m,:);%%%%%%  max
     mm=mm+1;
