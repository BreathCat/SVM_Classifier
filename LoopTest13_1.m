%%%%%%%%%%%%数据处理   求25种P帧内PU类型的概率为特征（25维）-----换归一化方式
Double_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\CIF\ALLP_PU\Double100K_400K\';
Single_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\CIF\ALLP_PU\Single400K\';
 %%%%%%   Double_featire
...
minnum=0;
for NUM=1:25
    maxnum=max(TrainData(:,NUM));
    minnum=min(TrainData(:,NUM));  
    [R,C]=size(TrainData)
    [R1,C1]=size(TestData)
    if (maxnum-minnum)>0
        TrainData(:,NUM)=(TrainData(:,NUM)-ones(R,1)*minnum)./(ones(R,1)*(maxnum-minnum));
        TestData(:,NUM)=(TestData(:,NUM)-ones(R1,1)*minnum)./(ones(R1,1)*(maxnum-minnum));
    end 
end 
    %训练分类器   
  %建立分类器模型
