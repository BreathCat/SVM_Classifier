...
Double_folder = 'C:\Users\45452\OneDrive - bjtu.edu.cn\HEVC\attack\libsvm\P-PUNUM\Ori_P-PUNum\100K\';%Single是Ori，Double是Henc
Single_folder = 'C:\Users\45452\OneDrive - bjtu.edu.cn\HEVC\attack\libsvm\P-PUNUM\Henc_P_PUNum\100K\';
half_num = 36; %Double_folder的个数
...
for iis=3:length(subfolders1)   %因为subfolders1的第一列和第二列是..所以iis从3开始
      ...
      Double_data_ave=mean(Double_data,1)*18/60 ;      %第iis个文件每一列平均成一列 1080P因为txt文件不规范，所以乘以系数
%       
        Double_feature_i=zeros(1,0);
        temp=(sum(Double_data));
        Double_feature_i(1,1)=temp(1,3);%3
        Double_feature_i(1,2)=temp(1,4);%4
        Double_feature_i(1,3)=temp(1,7);%7
...
for iis=3:length(subfolders2)  
      ...
      Single_data_ave=mean(Single_data,1)*18/60 ; %按列平均
...
%      Single_feature(mm,:)=Single_feature_prob;
        ...
        Single_feature_i(1,1)=temp(1,3);%3
        Single_feature_i(1,2)=temp(1,4);%4
        Single_feature_i(1,3)=temp(1,7);%7
