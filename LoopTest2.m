%%%%%%%%%%%%数据处理   取1,2,10,11,26,27这6种模式作为特征（150维）
Double_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\Double200K_300K\';
Single_folder = 'E:\梁晓云\video forensics\35intra_pred_modes\Singles300K\';
 %%%%%%   Double_featire
...
for iis=3:length(subfolders1)  
      ...
      nn=1;
      for i=1:m  %%%%%只取1,2,10,11,26,27这6种模式作为特征
          Double_feature_i(1,nn)=Double_data(i,1);
          Double_feature_i(1,nn+1)=Double_data(i,2);
          Double_feature_i(1,nn+2)=Double_data(i,10);
         Double_feature_i(1,nn+3)=Double_data(i,11);
         Double_feature_i(1,nn+4)=Double_data(i,26);
         Double_feature_i(1,nn+5)=Double_data(i,27);
         nn=nn+6;
      end
     mm=mm+1;
...
for iis=3:length(subfolders2)  
      ...
      pathname2 = [Single_folder subfolders2{iis}]; 
      Single_data=importdata(pathname2);
      [m,n]=size(Single_data);
      nn=1;
      for i=1:m
          Single_feature_i(1,nn)=Single_data(i,1);
          Single_feature_i(1,nn+1)=Single_data(i,2);
          Single_feature_i(1,nn+2)=Single_data(i,10);
         Single_feature_i(1,nn+3)=Single_data(i,11);
         Single_feature_i(1,nn+4)=Single_data(i,26);
         Single_feature_i(1,nn+5)=Single_data(i,27);
         nn=nn+6;
      end
     mm=mm+1;
     Single_feature(mm,:)=Single_feature_i;   %%%%     43*875
end   
