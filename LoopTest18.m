%%%%%%%%%%%%数据处理   求所有P帧内CU+PU类型的概率为特征（29维）
PU_Double_folder = 'E:\35intra_pred_modes\1080p\P_PUNum\Double10M-30M\';
PU_Single_folder = 'E:\35intra_pred_modes\1080p\P_PUNum\Single30M\';

CU_Double_folder = 'E:\35intra_pred_modes\1080p\P_CUNum\Double10M-30M\';
CU_Single_folder = 'E:\35intra_pred_modes\1080p\P_CUNum\Single30M\';
 %%%%%%   Double_featire
PU_t1 = dir(PU_Double_folder);% 先确定子文件夹 返回一个结构数组，包含了文件夹下的子文件夹和子文件的一些信息，第1个成员是文件名，第4个成员表示是否为文件夹。
PU_subfolders1 = {PU_t1.name};

CU_t1 = dir(CU_Double_folder);% 先确定子文件夹 返回一个结构数组，包含了文件夹下的子文件夹和子文件的一些信息，第1个成员是文件名，第4个成员表示是否为文件夹。
CU_subfolders1 = {CU_t1.name};
mm=0;
for iis=3:length(PU_subfolders1)  
      if PU_subfolders1{iis}==0
           continue
      end
      %%%%%%%%%%%%%%         PU
      PU_pathname1 = [PU_Double_folder PU_subfolders1{iis}]; 
      PU_Double_data=importdata(PU_pathname1);
      [m,n]=size(PU_Double_data);
      PU_Double_feature_i_sum=sum(PU_Double_data);
      PU_SUM=sum((sum(PU_Double_data))')
      PU_Double_feature_prob=PU_Double_feature_i_sum/PU_SUM;
      %%%%%%%%%%%%%%         CU
      CU_pathname1 = [CU_Double_folder CU_subfolders1{iis}]; 
      CU_Double_data=importdata(CU_pathname1);
      CU_Double_feature_i_sum=sum(CU_Double_data);
      CU_SUM=sum((sum(CU_Double_data))')
      CU_Double_feature_prob=CU_Double_feature_i_sum/CU_SUM;
 ...
 %%%%%%  Single_featire
 PU_t2 = dir(PU_Single_folder);% 先确定子文件夹 返回一个结构数组，包含了文件夹下的子文件夹和子文件的一些信息，第1个成员是文件名，第4个成员表示是否为文件夹。
PU_subfolders2 = {PU_t2.name};

CU_t2 = dir(CU_Single_folder);% 先确定子文件夹 返回一个结构数组，包含了文件夹下的子文件夹和子文件的一些信息，第1个成员是文件名，第4个成员表示是否为文件夹。
CU_subfolders2 = {CU_t2.name};
mm=0;
for iis=3:length(PU_subfolders2)  
      if PU_subfolders2{iis}==0
           continue
      end
      %%%%%%%%%%%%%%%           PU
      PU_pathname2 = [PU_Single_folder PU_subfolders2{iis}]; %%%%%    single
      PU_Single_data=importdata(PU_pathname2);
      PU_Single_feature_i_sum=sum(PU_Single_data);
      PU_SUM=sum((sum(PU_Single_data))')
      PU_Single_feature_prob=PU_Single_feature_i_sum/PU_SUM;
      
      %%%%%%%%%%%%%%%           CU
      CU_pathname2 = [CU_Single_folder CU_subfolders2{iis}]; %%%%%    single
      CU_Single_data=importdata(CU_pathname2);
      CU_Single_feature_i_sum=sum(CU_Single_data);
      CU_SUM=sum((sum(CU_Single_data))')
      CU_Single_feature_prob=CU_Single_feature_i_sum/CU_SUM;
